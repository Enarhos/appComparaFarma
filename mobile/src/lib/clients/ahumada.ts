...
export async function searchAhumada(
  query: string,
  signal?: AbortSignal
  ...
  while ((tile = tileRe.exec(html)) !== null) {
    ...
    const imgM = block.match(/<img[^>]+class="[^"]*tile-image[^"]*"[^>]*src="([^"]+)"|<img[^>]+src="([^"]+)"[^>]*class="[^"]*tile-image[^"]*"/);
    const imageUrl = imgM ? (imgM[1] ?? imgM[2] ?? null) : null;
    results.push({
      name,
      price,
      onlinePrice: null,
