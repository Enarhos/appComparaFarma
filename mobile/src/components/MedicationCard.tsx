...
import type { MedicationResult, PharmacyPrice, PharmacySlug } from "@/lib/types";
...
function getBestChannelLabel(price: PharmacyPrice, bestPrice: number): string {
  const { channels } = price;
  const config = PHARMACIES[price.pharmacySlug];
  if (channels.sbpay === bestPrice && config.sbpayLabel) return `📱 ${config.sbpayLabel}`;
  if (channels.cmr === bestPrice && config.cardLabel) return `💳 ${config.cardLabel}`;
  if (channels.online !== null && channels.online === bestPrice) return "🌐 Online";
  return "🏪 Presencial";
}
...
export function MedicationCard({ medication, activePharmacies }: MedicationCardProps) {
  ...
  // Canal del mejor precio
  const bestEntry = visiblePrices.find((p) => p.channels.effective === visibleBestPrice);
  const channelLabel = bestEntry ? getBestChannelLabel(bestEntry, visibleBestPrice) : null;

  // Badge de ahorro
  const worstEntry =
    visiblePrices.length > 1
      ? visiblePrices.reduce((max, p) =>
          p.channels.effective > max.channels.effective ? p : max
        )
      : null;
  const savings = worstEntry ? worstEntry.channels.effective - visibleBestPrice : 0;
  const savingsPharmacy = worstEntry
    ? PHARMACIES[worstEntry.pharmacySlug]?.name.replace("Farmacias ", "")
    : null;
        ...
        <View className="px-4 pt-4 pb-2">
          <View className="flex-row items-start justify-between gap-2">
            ...
            <View className="items-end gap-0.5">
              ...
              {channelLabel && (
                <Text className="text-xs text-green-500">{channelLabel}</Text>
              )}
            </View>
          </View>
