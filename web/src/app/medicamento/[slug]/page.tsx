...
import { computeSavings, sortByEffectivePrice } from "@comparafarma/domain";
...
/**
 * CF-WEB-002  una sola resoluci�n por request.
 *
 * `generateMetadata` y el componente de p�gina resolv�an el MISMO slug por
 * separado, as� que cada visita disparaba dos b�squedas en vivo independientes
 * contra las 9 farmacias. Adem�s de duplicar el costo, las dos pod�an diferir:
 * el cat�logo cambia entre llamadas, as� que la ficha pod�a renderizarse con un
 * producto mientras el `<title>`/`canonical` describ�an otro o directamente
 * decir "Medicamento no encontrado" sobre una ficha que s� resolvi�.
 *
 * `cache()` de React memoiza por request en el App Router: ambas rutas de
 * ejecuci�n ven exactamente la misma resoluci�n. Con la escalera de
 * recuperaci�n de `resolveMedicationBySlug` esto pasa de 2-4 b�squedas por
 * visita a 1-2.
 */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (resolution.status !== "ok") {
    // CF-WEB-002  el `noindex` es lo que hoy contiene el da�o SEO del
    // soft-404: la URL responde 200 (Next ya hizo flush del shell por el
    // `loading.tsx` del segmento ra�z, ver `docs/qa/cf-web-002/QA_SUMMARY.md`
    // �7 para la matriz de experimentos), as� que la �nica se�al efectiva
    // contra la indexaci�n de una ficha irresoluble es esta. NO cambiar por
    // `notFound()` ac�: se midi� y no altera el status, y s� har�a perder el
    // `noindex`.
    return {
