...
5. Si la decisión toca monetización, privacidad o modelo de negocio: consultá primero las fuentes ACTIVAS — `docs/product/PRODUCT_PRINCIPLES.md`, `docs/product/VISION.md`, `docs/product/decisions/DECISION_LOG.md`, y cualquier otra fuente vigente correspondiente al tema (ej. `docs/enterprise/` para modelo de negocio). `docs/archive/foundational-book/` (el "Libro Fundacional") es contexto histórico/filosófico y origen de los principios — consultalo para entender el porqué, pero nunca prevalece sobre una fuente activa (regla universal, `CLAUDE.md` §2: Archive nunca gobierna el presente). Si un principio importante solo existe en el Libro Fundacional y te parece que debería seguir siendo normativo, no lo apliques como si ya lo fuera — reportá `NEEDS_DECISION` para que Mario/ChatGPT decidan si se incorpora formalmente a una fuente activa.
...
- **No se prioriza nada que no exista antes en `docs/program/MASTER_BACKLOG.md`.** Si proponés algo nuevo que no existe en Program, proponé explícitamente su incorporación al backlog para decisión de Mario + ChatGPT — no lo agregues unilateralmente. Podés redactar la propuesta o el patch sugerido para `docs/program/MASTER_BACKLOG.md`, pero la incorporación de una iniciativa nueva requiere decisión CTO/Product.
