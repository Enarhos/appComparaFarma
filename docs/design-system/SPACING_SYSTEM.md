...
| **Nivel de Gobierno** | Estratégico — depende directamente de `docs/design-system/DESIGN_SYSTEM.md`, que ya identificó el espaciado como una Foundation pendiente de gobierno (§4.3 de ese documento), y de `docs/brand/BRAND_GUIDELINES.md` y `TYPOGRAPHY_SYSTEM.md` |
...
| **Documentos de los que depende** | `docs/design-system/DESIGN_SYSTEM.md`, `docs/brand/BRAND_GUIDELINES.md`, `docs/brand/TYPOGRAPHY_SYSTEM.md`, `docs/brand/VISUAL_IDENTITY.md`, `docs/brand/BRAND_FOUNDATIONS.md`, `docs/brand/BRAND_ARCHITECTURE.md`, `docs/brand/COLOR_SYSTEM.md`, `docs/design/DESIGN_BRIEF.md`, `docs/product/PRODUCT_PRINCIPLES.md`, `docs/templates/GOVERNED_DOCUMENT_TEMPLATE.md` |
...
`docs/design-system/DESIGN_SYSTEM.md` §4.3 ya identificó el espaciado como una Foundation de la arquitectura del Design System todavía sin gobierno propio. Este documento cierra ese vacío: no porque el espaciado sea un detalle estético pendiente, sino porque un sistema que ya exige que la jerarquía visual se apoye en el espaciado (`docs/brand/TYPOGRAPHY_SYSTEM.md` §4.5: "la jerarquía debe construirse combinando peso, tamaño relativo y espaciado") no puede dejar esa responsabilidad sin definición propia indefinidamente.
...
- La arquitectura tipográfica ni sus capas. Pertenece íntegramente a `docs/brand/TYPOGRAPHY_SYSTEM.md`, que este documento no duplica — solo referencia su relación (§4.4).
...
|---|---|---|
| Claridad | `docs/brand/BRAND_FOUNDATIONS.md` §11.1, Principio IV; `docs/design/DESIGN_BRIEF.md` §4.11 | El espacio debe reducir el esfuerzo de lectura de una comparación densa, nunca aumentarlo |
| Simplicidad | `docs/brand/BRAND_FOUNDATIONS.md` §11.2; `docs/design-system/DESIGN_SYSTEM.md` §4.2 | El sistema debe resolver la organización del espacio con el menor número de capas conceptuales necesarias — desarrollado en §4.2 |
| Consistencia | `docs/brand/BRAND_ARCHITECTURE.md` §4.1 (Branded House); `docs/design-system/DESIGN_SYSTEM.md` §4.2, §4.8 | Ninguna plataforma puede resolver una relación espacial de forma distinta a las demás sin registrar esa decisión como extensión formal del sistema |
| Jerarquía visual apoyada por el espacio | `docs/brand/TYPOGRAPHY_SYSTEM.md` §4.5 ("la jerarquía debe construirse combinando peso, tamaño relativo y espaciado"); `docs/brand/COLOR_SYSTEM.md` §4.3 | Fundamenta directamente por qué este documento existe: el espaciado ya es, por mandato de otro sistema, uno de los mecanismos obligatorios de jerarquía |
| Accesibilidad | `docs/brand/BRAND_FOUNDATIONS.md` §11.2; heredado de `TYPOGRAPHY_SYSTEM.md` §4.7 (interlineado, espaciado) y `COLOR_SYSTEM.md` §4.4 (redundancia visual) | Desarrollado en §4.6 |
...
El espacio en los bordes exteriores de una pantalla completa. Su responsabilidad es definir el límite entre el contenido del producto y el borde del dispositivo o ventana — es la capa más externa y, coherente con la prioridad mobile-first ya establecida (`docs/brand/TYPOGRAPHY_SYSTEM.md` §4.6), la primera que debe adaptarse cuando cambia el tamaño de pantalla.
...
- El espacio entre bloques de información permite distinguir un resultado de otro sin depender exclusivamente de percibir un cambio de color — el espaciado es, junto con la forma y el texto, uno de los mecanismos de redundancia visual que ya exige `docs/brand/COLOR_SYSTEM.md` §4.4 para que ningún significado dependa solo del color.
- Un ritmo espacial consistente (§4.3) reduce la carga cognitiva de tener que reinterpretar la estructura de cada pantalla nueva — mismo principio ya aplicado por analogía en `docs/brand/COLOR_SYSTEM.md` §4.3 ("el color debe reducir la carga cognitiva, no aumentarla").
...
`SPACING_SYSTEM.md` depende directamente de `docs/design-system/DESIGN_SYSTEM.md`, que identificó el espaciado como Foundation pendiente de gobierno y define la arquitectura de capas (Foundations → Tokens → Componentes → Patrones) dentro de la cual este documento se inserta como Foundation, no como Token. Depende también de `docs/brand/BRAND_GUIDELINES.md` (gobierno de convivencia entre sistemas), `docs/brand/TYPOGRAPHY_SYSTEM.md` (que ya exige espaciado como mecanismo de jerarquía) y `docs/brand/COLOR_SYSTEM.md` (cuyo principio de Neutralidad se extiende aquí a la organización espacial de comparaciones, §4.2.3).
...
| Espaciado como mecanismo obligatorio de jerarquía | `docs/brand/TYPOGRAPHY_SYSTEM.md` §4.5 | ✔ — fundamenta §4.1, desarrollado en §4.4 | — |
...
| Neutralidad entre farmacias | `docs/brand/BRAND_FOUNDATIONS.md` §12; `docs/brand/COLOR_SYSTEM.md` §4.5 | ✔ — extendida al Section spacing (§4.2.3) | Mismo riesgo de sesgo, distinto mecanismo (espacio en vez de color) |
| Redundancia visual | `docs/brand/COLOR_SYSTEM.md` §4.4 | ✔ — extendida al espaciado en §4.6 | — |
...
- `docs/brand/TYPOGRAPHY_SYSTEM.md` — sigue siendo la única fuente de arquitectura tipográfica; este documento solo señala su punto de coordinación necesaria (§4.4), sin resolverlo por su cuenta.
