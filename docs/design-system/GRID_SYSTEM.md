...
| **Documentos de los que depende** | `docs/design-system/DESIGN_SYSTEM.md`, `docs/design-system/SPACING_SYSTEM.md`, `docs/brand/BRAND_GUIDELINES.md`, `docs/brand/TYPOGRAPHY_SYSTEM.md`, `docs/brand/COLOR_SYSTEM.md`, `docs/brand/ICONOGRAPHY_SYSTEM.md`, `docs/brand/BRAND_FOUNDATIONS.md`, `docs/brand/BRAND_ARCHITECTURE.md`, `docs/design/DESIGN_BRIEF.md`, `docs/templates/GOVERNED_DOCUMENT_TEMPLATE.md` |
...
- El tratamiento del texto una vez que existe una región para él. Pertenece íntegramente a `docs/brand/TYPOGRAPHY_SYSTEM.md` (§4.5).
...
|---|---|---|
| Claridad | `docs/brand/BRAND_FOUNDATIONS.md` §11.1, Principio IV; `docs/design/DESIGN_BRIEF.md` §4.11 | El Grid es la primera capa donde este principio actúa — antes de cualquier otro sistema (§2) |
| Simplicidad | `docs/brand/BRAND_FOUNDATIONS.md` §11.2; `docs/design-system/DESIGN_SYSTEM.md` §4.2 | El sistema debe resolver la estructura con el menor número de capas conceptuales necesarias — desarrollado en §4.2 |
| Consistencia | `docs/brand/BRAND_ARCHITECTURE.md` §4.1 (Branded House); `docs/design-system/DESIGN_SYSTEM.md` §4.2, §4.8 | Fundamenta directamente el principio de Continuidad (§4.2.5) |
| Neutralidad | `docs/brand/BRAND_FOUNDATIONS.md` §11.2, §12; `docs/brand/COLOR_SYSTEM.md` §4.5 | Desarrollado íntegramente en §4.8 — extendido aquí de color y espacio a estructura |
| Accesibilidad | `docs/brand/BRAND_FOUNDATIONS.md` §11.2; heredado de `TYPOGRAPHY_SYSTEM.md` §4.7, `COLOR_SYSTEM.md` §4.4 y `SPACING_SYSTEM.md` §4.6 | Desarrollado en §4.7 |
...
- **Navegación:** el Grid reserva una Región funcional constante para la orientación dentro del producto — coherente con el concepto central "Orientación" (`docs/design/DESIGN_DECISION_LOG.md`, DD-001) y con la categoría de Navegación ya definida en `docs/brand/ICONOGRAPHY_SYSTEM.md` §4.2.1. Su posición debe ser predecible en cualquier pantalla, no solo consistente en sí misma.
- **Búsqueda:** el Grid reserva una Región funcional que antecede estructuralmente a los Resultados, porque la búsqueda es la entrada al mecanismo de decisión que el producto existe para servir (`docs/brand/BRAND_FOUNDATIONS.md` §7).
...
- **Mobile** es la condición primaria de diseño, mismo criterio ya establecido en `docs/brand/TYPOGRAPHY_SYSTEM.md` §4.6 y `docs/design/DESIGN_BRIEF.md` §4.11. La Estructura primaria (§4.2.1) debe definirse primero para esta condición más restrictiva y expandirse hacia arriba, nunca al revés.
...
Así como `docs/brand/COLOR_SYSTEM.md` evita sesgos mediante el color (§4.5 de ese documento) y `docs/design-system/SPACING_SYSTEM.md` evita sesgos mediante la proximidad (§4.2.3 de ese documento: "todas las secciones de resultado equivalentes deben usar el mismo Section Spacing"), el Grid debe evitar sesgos mediante la **organización estructural**.

**Principio arquitectónico:** ninguna farmacia debe ocupar sistemáticamente una posición privilegiada dentro de estructuras equivalentes sin una razón funcional explícita. El orden por precio efectivo ya calculado de forma objetiva por el sistema —`effective = min(store, online, cmr, sbpay)`, ya referenciado como "hecho ya calculado" en `docs/brand/COLOR_SYSTEM.md` §4.5— es una razón funcional explícita; una posición fija asignada por un acuerdo comercial no lo sería, y contradiría directamente `docs/brand/BRAND_FOUNDATIONS.md` §12 (*"no privilegiamos una farmacia por sobre otra por conveniencia comercial"*) y el Principio de producto 3, "Neutralidad" (§11.2).
...
`GRID_SYSTEM.md` depende directamente de `docs/design-system/DESIGN_SYSTEM.md`, que identificó el grid como Foundation pendiente de gobierno, y de `docs/design-system/SPACING_SYSTEM.md`, cuya distinción entre estructura y distancia (§4.5 de ese documento) este documento formaliza y desarrolla (§4.4). Depende también de `docs/brand/BRAND_GUIDELINES.md` (gobierno de convivencia entre sistemas), `docs/brand/TYPOGRAPHY_SYSTEM.md` (relación de facilitación de lectura, §4.5), `docs/brand/COLOR_SYSTEM.md` (cuyo principio de Neutralidad se extiende aquí a la estructura, §4.8) y `docs/brand/ICONOGRAPHY_SYSTEM.md` (categoría de Navegación, referenciada en §4.3).
...
| Principios de marca y producto aplicables | `docs/brand/BRAND_FOUNDATIONS.md`, `docs/product/PRODUCT_PRINCIPLES.md`, `docs/design-system/DESIGN_SYSTEM.md` §4.2 | ✔ — consolidados en §4.1 | Ningún principio nuevo agregado |
| Jerarquía de la información como parte de la identidad | `docs/design/DESIGN_BRIEF.md` §4.11 | ✔ — fundamenta §2 | — |
| Concepto central "Orientación" | `docs/design/DESIGN_DECISION_LOG.md`, DD-001 | ✔ — fundamenta la Región funcional de Navegación (§4.3) | — |
| Neutralidad entre farmacias | `docs/brand/BRAND_FOUNDATIONS.md` §12, §11.2; `docs/brand/COLOR_SYSTEM.md` §4.5; `docs/design-system/SPACING_SYSTEM.md` §4.2.3 | ✔ — extendida a la estructura en §4.8 | Tercera aplicación del mismo principio, tras color y espacio |
...
| Capas tipográficas y su compatibilidad con regiones estructurales | `docs/brand/TYPOGRAPHY_SYSTEM.md` §4.2 | ✔ — relación explicada en §4.5, sin duplicar sus capas | — |
| Categoría de Navegación en iconografía | `docs/brand/ICONOGRAPHY_SYSTEM.md` §4.2.1 | ✔ — referenciada en §4.3 | — |
...
- `docs/brand/TYPOGRAPHY_SYSTEM.md` — sigue siendo la única fuente del tratamiento tipográfico; este documento solo señala su punto de coordinación (§4.5).
