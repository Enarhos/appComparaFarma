...
| Khipu | Comisión por transacción (~0,69%+IVA, no confirmado en página oficial) | Donaciones: link estático en Mobile (deshabilitado) + CTA en Web vía `/api/donate` (Instant Payments API 3.0, `KHIPU_PAYMENT_CREATION: VERIFIED_IN_PRODUCTION`) — **CTA Web pausado temporalmente desde 2026-08-16, ver nota abajo** | `HUMAN_ACTION_REQUIRED` | 🟢 Bajo (bajó de 🟡 tras la pausa) — donaciones Web pausadas mitigan el uso comercial activo sobre Vercel Hobby; confirmación de pago sigue no implementada (`KHIPU_PAYMENT_CONFIRMATION: NOT_IMPLEMENTED`); credenciales API 2.0 conservadas como `LEGACY_ROLLBACK_ONLY` sin rotar; webhook pendiente por secreto de firma no confirmado | Rotación de credenciales API 2.0 sigue pendiente por separado; reactivar CTA Web solo junto con el upgrade a Vercel Pro | CTO |
| Upstash Redis | Free | Caché de búsqueda + rate limiting distribuido | `PRODUCTION_VERIFIED` / `OK` (resuelto e incidente cerrado 2026-08-16, ver nota abajo) | 🟢 Bajo — incidente cerrado: la base Free anterior (`comparafarma-cache`) fue eliminada automáticamente por Upstash tras el período de inactividad del plan Free; se creó una nueva base `comparafarma-cache` (Upstash Free, AWS us-east-1, REST API + Persistence + TLS habilitados), se reemplazaron `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` en Vercel (Production y Preview) y se hizo un redeploy limpio de Production. Verificado en vivo: `/api/health` → `redis: "ok"`. Fallback a memoria sigue disponible como red de seguridad, sin cambios | Monitorear consumo de comandos/mes; considerar que el plan Free elimina la base tras ~14 días de inactividad si el tráfico vuelve a bajar a cero | CTO |
...
> **Actualización (2026-08-16):** decisión de producto ya tomada (no reabierta en este cierre): pausar también las donaciones de **Web** mientras ComparaFarma opera en Vercel Hobby, para no mantener uso comercial activo (donaciones) sobre un plan que lo prohíbe explícitamente (§ tabla, fila Vercel) mientras no se resuelva el upgrade a Pro. Implementado de forma reversible mediante una constante explícita `WEB_DONATIONS_PAUSED` (`web/src/lib/donationsConfig.ts` y `api/src/lib/donationsConfig.ts`, en `true` por defecto): el `Footer` global deja de renderizar `DonationWidget` y muestra en su lugar el texto neutro "Los aportes están temporalmente pausados mientras ComparaFarma se encuentra en su etapa inicial de crecimiento." (sin pedir dinero, sin botón Khipu); `createDonationPayment` (Server Action) corta antes de llamar a `/api/donate`; y `/api/donate` responde `503` con un mensaje explícito antes de tocar rate limiting o crear cualquier pago Khipu. **No se eliminó** la integración Khipu, `/api/donate`, los adapters, los tests, ni las variables de entorno — reactivar es flipear ambas constantes a `false` y hacer un nuevo deploy, sin reconstruir nada. Con Mobile ya pausado desde 2026-08-15, **las donaciones están hoy pausadas en ambas plataformas** por la misma razón de fondo (Mobile: revisión tributaria pendiente + adquisición inicial; Web: uso comercial sobre Vercel Hobby) — ambas quedan condicionadas a resolver la migración de Vercel a un plan compatible con uso comercial antes de reactivarse.
...
5. **Servicios que requieren la credencial nueva (`KHIPU_API_KEY`):**
   ...
   | Web | No — el CTA de Web (`DonationWidget`/`createDonationPayment`) usa `KHIPU_API_KEY` vía `/api/donate`, no una credencial propia; y desde 2026-08-16 el CTA está pausado (`WEB_DONATIONS_PAUSED`), sin llamar a Khipu | NO |
...
7. **Riesgo de hacerlo:** bajo — `/api/donate` no tiene hoy ningún llamador real activo en producción (Mobile deshabilitado desde 2026-08-15; Web pausado desde 2026-08-16 vía `WEB_DONATIONS_PAUSED`, responde 503 antes de invocar Khipu), así que un error de configuración no afecta a ningún usuario real.
...
| 1.4 | 2026-08-16 | Activo | Pendiente (CTO) | Cierre de producción (rama fix/production-closure-upstash-web-donations): (1) diagnosticado y documentado el estado real de Upstash — variables sí configuradas en Vercel, `/api/health` reporta `not_configured` por `STALE_DEPLOYMENT` (deploy anterior a la carga de las variables), sin cambio de código, requiere redeploy manual; (2) donaciones Web pausadas temporalmente (`WEB_DONATIONS_PAUSED`, reversible) mientras se opera en Vercel Hobby, mismo criterio que la pausa de Mobile del 2026-08-15 — sin eliminar integración Khipu, `/api/donate`, adapters, tests ni variables de entorno. |
| 1.5 | 2026-08-16 | Activo | Pendiente (CTO) | Corrección de revisión CTO: la conclusión `STALE_DEPLOYMENT` para Upstash (v1.4) quedó refutada por evidencia nueva — un deployment de `comparafarma-api` posterior a la creación de las env vars sigue sin exponer Redis (`/api/health` re-verificado en vivo, `not_configured`). Reclasificado a `UNCONFIRMED`; se especifica exactamente qué dato del Dashboard de Vercel falta para cerrar el diagnóstico. Sin cambio de código de producción — solo corrección de comentarios de test y de este documento. El registro de donaciones Web pausadas (v1.4) no cambia. |
| 1.6 | 2026-08-16 | Activo | Pendiente (CTO) | Cierre del incidente Upstash: causa raíz confirmada — la base Free anterior `comparafarma-cache` fue eliminada automáticamente por Upstash tras el período de inactividad del plan Free (`UPSTASH_ROOT_CAUSE = DELETED_FREE_UPSTASH_DATABASE`). Resuelto creando una nueva base `comparafarma-cache` (Upstash Free, AWS us-east-1, REST API + Persistence + TLS), reemplazando `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` en Vercel (Production y Preview) y haciendo un redeploy limpio de Production. Verificado en vivo: `/api/health` → `redis: "ok"`. `UPSTASH_STATUS = PRODUCTION_VERIFIED`. Se retiró el diagnóstico temporal `/api/health?debug=1` (presencia booleana de env, agregado en v.anterior para investigar el incidente) — `api/src/routes/health.ts` vuelve a su contrato normal; se conserva `api/src/__tests__/cache.test.ts` por su cobertura válida del fallback Redis/memoria. Registro de donaciones Web pausadas sin cambios. |
