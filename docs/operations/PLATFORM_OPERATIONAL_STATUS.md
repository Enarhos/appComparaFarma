...
| Upstash Redis | Free | Caché de búsqueda + rate limiting distribuido | `UPSTASH CONFIG PRESENT / RUNTIME STATUS UNRESOLVED` (revisado 2026-08-16 — ver nota abajo) | 🟡 Medio — `/api/health` en producción sigue reportando `not_configured` (confirmado en un chequeo en vivo posterior a un deployment de `comparafarma-api` creado después de que las env vars ya existían en Vercel, lo que descarta un simple deploy desactualizado como explicación suficiente); causa raíz real `UNCONFIRMED` — código de `cache.ts`/`health.ts` verificado sin bug. Fallback a memoria sigue verificado y activo, nunca bloquea funcionalidad | Revisar en el Dashboard de Vercel: qué deployment está realmente promovido/aliaseado a `comparafarma-api.vercel.app`, si ese deployment específico tiene `UPSTASH_REDIS_REST_URL`/`TOKEN` adjuntas, y si esas env vars están limitadas a algún "Environment" o rama específica distinta de la que sirve producción | CTO |
