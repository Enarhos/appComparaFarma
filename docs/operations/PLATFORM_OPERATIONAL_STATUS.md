...
| Upstash Redis | Free | Caché de búsqueda + rate limiting distribuido | `HUMAN_ACTION_REQUIRED` (actualizado 2026-08-16 — ver nota abajo) | 🟡 Medio — `/api/health` en producción reporta `not_configured` pese a que `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` ya están configuradas en Vercel (Production y Preview); causa raíz diagnosticada como `STALE_DEPLOYMENT` (el último deploy es anterior a la carga de esas variables, y Vercel solo las inyecta en builds nuevos). Fallback a memoria sigue verificado y activo, nunca bloquea funcionalidad | Disparar un redeploy de `comparafarma-api` en Vercel y confirmar `/api/health` → `redis: "ok"` | CTO |
