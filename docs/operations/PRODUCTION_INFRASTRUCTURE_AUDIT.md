...
| 3 | Supabase (Postgres + Auth) | `api/package.json` (`@supabase/supabase-js`), `web/package.json` (`@supabase/ssr`), `mobile/package.json` (`@supabase/supabase-js`), `api/src/lib/supabaseClient.ts`, `docs/technology/database/schema.sql`, hecho confirmado en vivo por el CTO (Dashboard → Authentication → Rate Limits) |
...
| 10 | Flow (Chile, pagos recurrentes) | `api/src/lib/adapters/flowAdapter.ts`, `api/.env.example` (`FLOW_API_KEY`/`FLOW_SECRET_KEY`/`FLOW_API_BASE_URL`), `docs/technology/decisions/adr/ADR-0004_FLOW_SUBSCRIPTION_INTEGRATION.md` |
...
| 19 | Keystore de firma Android (`release.keystore`) | `docs/operations/RUNBOOK.md` §5, §8; `docs/archive/releases/RELEASE_CHECKLIST.md` (verificación SHA-1/SHA-256) |
...
4. **Evidencia:** `.github/workflows/ci.yml` líneas del job `deploy-api` (`VERCEL_ORG_ID: team_QtbvbI6hTSxxSJ9qDFTv9z6S`, `VERCEL_PROJECT_ID: prj_zvHG2urEOjMM770FPy6B2fdhk915`, `vercel deploy --prod --yes`); `api/vercel.json` (`"functions": {"api/*.ts": {"maxDuration": 30}}`); `docs/technology/postmortems/PM-001_DEPLOY_PIPELINE_BROKEN.md` confirma el error real "No more than 12 Serverless Functions can be added to a Deployment on the Hobby plan" obtenido en producción.
...
10. **Recomendación:** Mantener Hobby mientras el tráfico sea bajo, pero planificar el upgrade a Pro *antes* de cualquier campaña de adquisición de usuarios significativa — no como reacción a una caída. Agregar el smoke test post-deploy ya implementado (ver `docs/archive/releases/RC-03_PRODUCTION_READINESS_REPORT.md`) es una mitigación correcta ya en marcha, mantenerla.
...
10. **Recomendación:** Igual que Flow — confirmar con el CTO si se espera que RTDN esté activo hoy; si sí, configurar `GOOGLE_RTDN_SECRET` en Vercel y el tópico de Pub/Sub en Play Console (procedimiento en `docs/program/backlog/issues/CF-114_GOOGLE_PLAY_ADAPTER.md`).
...
| Variables de entorno | Documentadas exhaustivamente y con "impacto si falta" explícito para cada una en `docs/operations/environment/ENVIRONMENT.md` — nivel de documentación notablemente alto comparado con proyectos similares. | `docs/operations/environment/ENVIRONMENT.md` completo. |
| Fail-open vs. fail-closed | `API_SECRET_KEY` es explícitamente **fail-open** (si no está configurada, `/api/search` queda sin autenticación) — riesgo conocido y documentado, no un descuido. `CRON_SECRET`/`GOOGLE_RTDN_SECRET` son fail-closed por diseño (correcto). | `api/src/middleware/auth.ts`, `docs/operations/environment/ENVIRONMENT.md` sección "Variables de mayor riesgo operacional". |
...
| Logging | Existe una capa de saneamiento (`api/src/lib/logger.ts`, creada en sprint RC-03) que redacta emails y tokens largos antes de loguear — aplicada en 13 archivos según `docs/archive/releases/RC-03_PRODUCTION_READINESS_REPORT.md`. | `docs/archive/releases/RC-03_PRODUCTION_READINESS_REPORT.md` sección "Cambios implementados → Logging". |
| Monitoreo/Observabilidad | Monitor horario de 9 farmacias + `/api/health` (`monitor-api.yml`), Sentry condicional en ambos frontends de código (api/mobile), pero — **hallazgo verificado en esta auditoría** — el `/api/health` real en el código (`api/src/routes/health.ts`) solo devuelve `{ ok, service, timestamp }`, **sin** los campos `environment`, `commit`, `uptimeSeconds`, `memoryMb`, `dependencies.{redis,supabase,algolia}` que `docs/operations/RUNBOOK.md` (§6) y `docs/archive/releases/RC-03_PRODUCTION_READINESS_REPORT.md` (sección "Health Check") afirman que fueron implementados. Esto es una discrepancia real entre documentación y código verificada línea por línea en esta sesión, no una suposición — o bien el cambio se revirtió después de RC-03, o el deploy actual no lo refleja. **Tiene impacto operacional directo**: el RUNBOOK usa `/api/health` → `dependencies.supabase`/`dependencies.redis` como paso de diagnóstico en su tabla de incidentes (§3), un procedimiento que no puede ejecutarse tal como está escrito hoy. | `api/src/routes/health.ts` (contenido completo leído, 11 líneas), comparado con `docs/operations/RUNBOOK.md` §6 y `docs/archive/releases/RC-03_PRODUCTION_READINESS_REPORT.md`. |
...
| Cuentas propietarias | Ver sección Single Points of Failure #1 — concentración documentada y reconocida por el propio proyecto, no un hallazgo nuevo de esta auditoría, pero sí uno que sigue sin resolverse según la evidencia (`ADMIN_ALLOWED_EMAILS` sigue siendo un único email). | `docs/archive/releases/SERVICE_ACCOUNT_MIGRATION.md`, `web/.env.example`. |
