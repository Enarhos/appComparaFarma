...
5. **Retención tras dejar de ser Usuario — RESUELTO (2026-08-21, AUTH-DELETE-01):** se eliminan. Política activa: DELETE por defecto sobre todo dato personal sin finalidad vigente (`auth.users`, `profiles`, `subscriptions`, `subscription_events.raw_payload`, `flow_customers`, `email_alerts`/`feedback` correlacionables por email) — ningún dato personal se "conserva agregado" sin un fundamento legal documentado (no existe ninguno hoy, ver GATE_3_AUTH_DELETE_01_REPORT.md). `price_history`/`pharmacy_clicks` (activos de plataforma, no vinculados a una persona) no se ven afectados — no son parte de esta decisión, nunca lo fueron. Diseño y mecanismo (`deletion_pending`, RPC `delete_account_data`) en `docs/technology/database/schema.sql` y `api/src/routes/account.ts`.
