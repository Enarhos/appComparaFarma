...
- **Subscription Platform — Fase 2 (Web Billing + Stripe)** — ✅ Implementado y mergeado a `main` (2026-08-02). CFPS registrado el mismo día, autorizado a ejecutar directamente por el CEO (papeleo + implementación en el mismo pedido, sin pausa de ratificación intermedia como en Fase 1).
  ...
  **CFPS = (2×0.25)+(5×0.15)+(2×0.20)+(5×0.20)+(3×0.10)+(3×0.05)+(2×0.05) = 3.20 — Media (evaluar), mismo rango que Fase 1 (3.0) y CFM-ID (3.2).**
  ...
  #### ⚠️ Corrección (2026-08-02, mismo día): Stripe no admite Chile — reemplazado por Flow

  Al intentar crear la cuenta real de Stripe, se confirmó que **no acepta comercios domiciliados en Chile** (verificado oficialmente en `stripe.com/global`) — bloqueador real, no de configuración. Se evaluó Khipu (bloqueada: su API de pagos automáticos requiere gestión comercial previa con el equipo de ventas, no es self-service) y se optó por **Flow**, cuenta ya creada por el CEO, self-service. El contrato completo de la API de suscripciones de Flow se verificó manualmente de punta a punta en su sandbox (plan → cliente → tarjeta de prueba enrolada → suscripción → primer cobro confirmado pagado → payload real del webhook capturado con `webhook.site`) antes de diseñar el reemplazo, sin adivinar ningún nombre de campo.

  Documentado en `docs/engineering/rfc/RFC-005_WEB_BILLING_FLOW.md` y `docs/engineering/adr/ADR-0004_FLOW_SUBSCRIPTION_INTEGRATION.md`, issues `CF-122` a `CF-127`. RFC-004/ADR-0003/CF-117 a CF-121 quedan marcados **Superseded** (no eliminados — registro histórico de la decisión y el diseño). Mismo CFPS 3.20 — es corrección de proveedor sobre el mismo alcance ya ratificado, no una fase nueva (ver RFC-005 §7).

  **✅ Ratificado y ejecutado (2026-08-03)**: CEO ratificó RFC-005/ADR-0004 sin ajustes. Entregado: tabla aditiva `flow_customers` + retiro de `stripe_price_id` (`docs/database/schema.sql`, sección "Fase 2 corregida"); `api/src/lib/adapters/flowAdapter.ts` (firma HMAC-SHA256, cliente HTTP de Flow, resolución del webhook `token`-únicamente vía `GET /payment/getStatus`, parsing de `commerceOrder`); 3 acciones nuevas en el endpoint consolidado (`start-flow-subscription`, `flow-register-return`, `flow-webhook`, sin sumar funciones Vercel); `web/`: `startFlowSubscription.ts` (Server Action) + `UpgradeButton.tsx` adaptado + banner `?upgrade=success|error` en `/cuenta`. Código de Stripe eliminado por completo de `main` (`stripeAdapter.ts`, `createCheckoutSession.ts`, acciones/columna asociadas) — decisión explícita del CEO. 30 tests nuevos/actualizados en `api/` (172 tests totales verdes) + 8 en `web/`; `pnpm typecheck` limpio en `api/`/`web/`. Contrato de Flow verificado manualmente de punta a punta en su sandbox antes de escribir el adaptador (plan → cliente → tarjeta de prueba → suscripción → primer cobro pagado → webhook real capturado). **Pendiente de Mario**: correr en Supabase la sección "Fase 2 corregida" de `schema.sql` (`flow_customers`, retiro de `stripe_price_id`); configurar `FLOW_API_KEY`/`FLOW_SECRET_KEY`/`FLOW_API_BASE_URL` en Vercel con las credenciales de la cuenta **productiva** de Flow (distintas de las de sandbox usadas en la verificación) y `API_PUBLIC_URL`; definir y crear el primer plan real vendible (fila en `subscription_plans` con `is_available=true`).
