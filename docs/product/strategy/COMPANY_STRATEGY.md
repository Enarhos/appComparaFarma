...
|---|---|---|---|
| **1** ✅ **Completa (2026-07-20)** | Base de datos (Supabase): `price_history` (historial server-side, esquema genérico de canales) + `pharmacy_clicks` vía `/api/go` (tracking de clicks, base para afiliación, con validación de dominio contra open redirect) — ambas verificadas en producción con datos reales | Nada — es el punto de partida | No — vive entero en `api/`, invisible para mobile |
| **2a** ✅ **Completa (2026-07-20)** | Web con SEO (Next.js): Home + `/buscar/[query]` con metadata dinámica, imagen + desglose de canales por farmacia (paridad con la app), consumiendo el mismo `api/` — deployado en producción en `https://app-compara-farma-web.vercel.app`, verificado con datos reales (tracking de clicks `/api/go` incluido). SEO reforzado: `sitemap.xml` + `robots.txt` (búsquedas frecuentes curadas) + JSON-LD `Product`/`AggregateOffer` en `/buscar/[query]` — verificado en producción con la URL real | Fase 1 (DB) | No — proyecto nuevo `web/`, separado |
...
| **3** 🟡 **En curso (2026-07-20)** | Panel admin en `/admin` dentro de `web/`, protegido con Supabase Auth (Google + email/contraseña, lista blanca `ADMIN_ALLOWED_EMAILS`). **Dashboard de clicks** ✅ verificado en producción. **Config de farmacias/banner** y **bandeja de feedback**: código completo (`app_config`/`feedback` en Supabase, reemplazan env vars DISABLED_PHARMACIES/DONATION_BANNER_*, contrato de `/api/config` sin cambios), typecheck+build+tests limpios, deployado — pendiente que el usuario corra la sección "Fase 3" de `docs/technology/database/schema.sql` en el SQL Editor de Supabase para que dejen de mostrar error de "tabla no existe" (mientras tanto el backend sigue funcionando con las env vars de siempre, cero impacto público). Llaves de API B2B: no iniciado, sigue gateado por volumen de datos | Fase 1 (DB) | No |
