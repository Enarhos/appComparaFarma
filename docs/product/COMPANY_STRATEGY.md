...
| **3** 🟡 **En curso (2026-07-20)** | Panel admin en `/admin` dentro de `web/`. Primer alcance elegido por el usuario: **dashboard de clicks por farmacia** (datos de Fase 1), protegido con Supabase Auth (login con Google + email/contraseña, lista blanca `ADMIN_ALLOWED_EMAILS`) — verificado en producción con datos reales. Pendiente del alcance más amplio de Fase 3: config de farmacias/banner (hoy env vars), bandeja de feedback, y llaves de API B2B (esto último sigue gateado por volumen de datos) | Fase 1 (DB) | No |
