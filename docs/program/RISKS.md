...
| **Documentos de los que depende** | `docs/product/RISKS.md`, `docs/release/PRODUCTION_READINESS_V2.md`, `docs/brand/BRAND_ARCHITECTURE.md` §4.8, `docs/analysis/PROJECT_INVENTORY.md` |
...
| R-011 | Ejecución de la migración SQL del Registro Canónico (CFM-ID) en producción no confirmada en la evidencia revisada | Medio — si no corrió, el sistema funciona igual pero sin continuidad de identidad de medicamento | No determinable | Confirmar directamente en Supabase | 🟡 Abierto, no verificable | `docs/analysis/PROJECT_INVENTORY.md` §3 |
| R-012 | "Marketplace futuro" (Digital Asset Register, DAR-400) contradice el posicionamiento declarado "no somos un marketplace" (Vision 2030, Brand Foundations) | Medio — riesgo de incoherencia de marca si se construye sin resolver la tensión | Baja (todavía no se ha empezado a construir) | Ninguna — tensión detectada, no resuelta | 🟡 Abierto | `docs/brand/BRAND_ARCHITECTURE.md` §4.8 |
| R-013 | Inconsistencia terminológica entre `BUSINESS_CAPABILITY_MAP.md` y `BUSINESS_SERVICES.md` (nombres de al menos 4 capacidades citados de forma distinta) | Medio — si se traslada a naming público, erosiona los atributos de percepción "Confianza"/"Evidencia" | Baja (interna, no pública todavía) | Ninguna — reportado, no corregido | 🟡 Abierto | `docs/brand/BRAND_ARCHITECTURE.md` §4.4 |
| R-014 | Ninguna decisión de marca o diseño (identidad, logo, color) tiene ratificación formal del CEO | Medio — cualquier trabajo posterior de diseño se apoya en una base no ratificada | Alta (es el estado actual, no una posibilidad futura) | Ninguna — pendiente de agenda del CEO | 🔴 Abierto | `docs/brand/BRAND_FOUNDATIONS.md`, `docs/design/*` |
| R-015 | Touchpoint de marca externa (Flow) visible durante el checkout de suscripción, sin criterio documentado de convivencia con la marca ComparaFarma | Bajo | Media | Ninguna documentada | 🟡 Abierto | `docs/brand/BRAND_ARCHITECTURE.md` §4.8 |
| R-016 | Historial de precios con dos mecanismos de datos no unificados entre mobile (local) y web/api (Supabase) | Bajo — inconsistencia de datos entre plataformas, no visible como error | Media | Ninguna | 🟡 Abierto | `docs/analysis/PROJECT_INVENTORY.md` §9 |
| R-017 | Keystores de firma Android (3 `.jks`) sueltos en el filesystem del proyecto, no trackeados en git | Bajo | Baja | `.gitignore` ya excluye estos archivos del control de versiones | 🟢 Mitigado | `docs/release/PRODUCTION_READINESS_V2.md` §5 |

---

## 5. Relaciones

Este registro consolida `docs/product/RISKS.md`, la sección 3 de `docs/release/PRODUCTION_READINESS_V2.md`, la sección 4.8 de `docs/brand/BRAND_ARCHITECTURE.md`, y los puntos marcados "incierto" en `docs/analysis/PROJECT_INVENTORY.md`. No reemplaza a ninguno como fuente de detalle.
...
| Riesgos de release/Google Play | `docs/release/PRODUCTION_READINESS_V2.md` | ✔ (R-001 a R-004) | — |
| Riesgos de marca/arquitectura | `docs/brand/BRAND_ARCHITECTURE.md` §4.8 | ✔ (R-012 a R-015) | — |
| Incertidumbres de estado real | `docs/analysis/PROJECT_INVENTORY.md` | ✔ (R-011, R-016) | — |
...
`docs/product/RISKS.md`, `docs/release/PRODUCTION_READINESS_V2.md`, `docs/brand/BRAND_ARCHITECTURE.md`, `docs/program/PROGRAM_BOARD.md`, `DECISION_QUEUE.md`.
