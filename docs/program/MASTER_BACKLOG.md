...
| **CF-QA-001** — Campaña de QA "Search Product Identity & Price Comparison" (10 hallazgos + certificación de `fix/quantity-mismatch-false-merge`) | Alta (incluye 2 hallazgos P1) | ✅ **Campaña de QA cerrada** — evidencia completa pusheada en branch `docs/qa-search-product-identity-001` (SHA `e09f9e2`), **no mergeada a `origin/main`** (pendiente de PR propio). El PR bajo prueba, `fix/quantity-mismatch-false-merge`, fue certificado `SAFE_TO_MERGE` por esta campaña y **ya fue mergeado** a `origin/main` (merge commit `48a7de5`) | Ninguna para el registro de los 10 hallazgos; PR propio pendiente para mergear la evidencia de QA (`docs/qa/`) a `main`; decisión CTO/Product para priorizar los 10 issues candidatos | `docs/qa/search-product-identity/QA_SUMMARY.md`, `cases/QA-SEARCH-001.md` a `010.md`, `analysis/`, `raw/` (todo en la branch `docs/qa-search-product-identity-001`, no en `origin/main` todavía) |
...
**Estado:** campaña de QA cerrada (`CF-QA-001 — Search Product Identity & Price Comparison QA`). Evidencia completa pusheada en la branch `docs/qa-search-product-identity-001` (commit `e09f9e2`), en `docs/qa/search-product-identity/` (`QA_SUMMARY.md`, `cases/QA-SEARCH-001.md` a `QA-SEARCH-010.md`, `analysis/`, `raw/`). **Esta branch de evidencia está pusheada a `origin` pero no mergeada a `origin/main`** — pendiente de su propio PR/review. Esta entrada registra en backlog los hallazgos de la campaña; no implementa ninguno de ellos.

**Propósito de la campaña:** auditar dos entornos — producción actual (`origin/main` en `acd79bf`, que ya incluye CF-SEARCH-001/PR #132, CF-SEARCH-002/PR #133 y BIOEQUIVALENCE-DATA-QUALITY-01/PR #141) y el PR entonces bajo prueba `fix/quantity-mismatch-false-merge` (`b20402a`) — contra la pregunta central: *¿ComparaFarma compara precios solo entre productos/presentaciones que razonablemente corresponden al mismo producto?* Metodología: `GET /api/search` público (sin `?debug=1`, sin credenciales), 29 consultas (13 del alcance mínimo del ticket + 16 de estrés de formas farmacéuticas), 2.347 tarjetas / 2.627 ofertas analizadas, 9/9 farmacias cubiertas, A/B de `mergeDuplicates()` sobre los mismos datos RAW de producción pasados por ambos builds de `@comparafarma/domain`.

**Resultado de los 12 tests de la campaña:** 4 PASS (cantidad, forma farmacéutica, integridad de oferta, variante comercial) · 5 FAIL (dosis/concentración, navegación, laboratorio/marca, nombres truncados, false splits) · 3 WARNING (marca no identificada, precio sospechoso, stock). Detalle completo y metodología de cálculo: `QA_SUMMARY.md` en la branch de evidencia.

**Certificación de `fix/quantity-mismatch-false-merge` — `SAFE_TO_MERGE`.** La campaña certificó el PR como seguro para mergear: A/B sobre 2.281 ofertas únicas de las 9 farmacias mostró 0 diferencias de tarjetas entre `origin/main` y el PR (2.022 tarjetas idénticas en ambos entornos); alcance limpio (5 archivos, todo dentro de `packages/domain`, no toca `ci.yml`/`api/vercel.json`/contrato público, no requiere bump de `CACHE_PREFIX`); 975 tests verdes y typecheck limpio, verificados por QA, no solo reportados. Este PR **ya fue mergeado** a `origin/main` (merge commit `48a7de5`, ver `git log origin/main`). La campaña dejó constancia honesta de que el defecto original reportado ("1 sobre comparado contra una caja de 6") no es reproducible en los datos de producción de hoy — el PR es una red de seguridad correcta para un fallo que hoy no ocurre, no la corrección de un defecto activo — y que introduce un riesgo latente propio (ver QA-SEARCH-008 abajo).

**Los 10 hallazgos registrados** (severidad y evidencia — detalle completo de cada uno en `docs/qa/search-product-identity/cases/QA-SEARCH-0XX.md`, branch de evidencia):

| ID | Título | Severidad | Introducido por el PR bajo prueba | Medición |
|---|---|---|---|---|
| QA-SEARCH-001 | Concentración incompatible fusionada en formas líquidas (`matching.ts` prioriza volumen sobre mg) | **P1** | No — preexistente | 5 tarjetas con dos concentraciones mg explícitas y distintas, todas `fluid-oral` (ej. Betametasona 0.25mg vs 2mg, factor 8x) |
| QA-SEARCH-002 | Fichas de producto irresolubles con HTTP 200 (soft-404 en `resolveMedication.ts`) | **P1** | No — preexistente | 7 de 48 enlaces de ficha (14,6 %) no resuelven; todos responden HTTP 200 con "Medicamento no encontrado" |
| QA-SEARCH-003 | 90,5 % de las tarjetas comparan una sola farmacia (`commercialIdentity.ts`) | P2 | No — preexistente | 270 grupos farmacológicamente equivalentes repartidos sin solapamiento de farmacias (346 grupos fragmentados, 923 tarjetas) |
| QA-SEARCH-004 | Semántica inconsistente del campo `laboratory` entre farmacias (`api/src/clients`) | P2 | No — preexistente | Salcobrand: 83,5 % es el prefijo del nombre del producto, no un laboratorio real (333/399); Cruz Verde: mapeo muerto (campo fuente inexistente); 5 de 9 farmacias siempre `null` |
| QA-SEARCH-005 | Tokens no-marca en el segmento `brand:` de `presentationKey` | P2 | No — preexistente | Consecuencia directa de QA-SEARCH-004; 28 tokens `brand:` inválidos identificados |
| QA-SEARCH-006 | "Mejor precio" agotado sin evidencia de stock, mostrado como disponible | P2 | No — preexistente | 375 tarjetas (16 %) con `bestPrice` de oferta agotada; 2 de 9 adaptadores hardcodean `hasStock: true` |
| QA-SEARCH-007 | EasyFarma: 68,2 % de nombres truncados en origen (`easyfarma.ts`, `SOURCE_TRUNCATION`) | P3→P2 | No — preexistente | 167 de 245 ofertas de EasyFarma truncadas; 0 en las otras 8 farmacias |
| QA-SEARCH-008 | `unitCountKey()` deriva cantidades falsas en 3 patrones de nombre reales (falso split latente) | P2 | **Sí** — introducido por `fix/quantity-mismatch-false-merge` | Ej. "x 10unidades"→1, "4Un"→1, "25-25"→2525, "200 Ds"→200; delta 0 hoy (ninguno colisiona todavía), riesgo crece si mejora el eje `brand:` (QA-SEARCH-003/005) |
| QA-SEARCH-009 | Sinónimos de variante comercial no unificados (familia Tapsin) | P3 | No — preexistente | Verificado sobre 121 tarjetas de Tapsin (9 multi-oferta, 23 variantes) |
| QA-SEARCH-010 | Ranking de relevancia no distingue bien cosmética vs medicamento en consultas ambiguas (`relevance.ts`) | P3 | No — preexistente | Ver caso QA-SEARCH-010 en la branch de evidencia |

**Recomendación explícita de la campaña sobre QA-SEARCH-008:** resolverlo (como `CF-SEARCH-006`, ver abajo) **antes** de mejorar el eje `brand:` (QA-SEARCH-004/005), porque es una regla dura alimentada por un lector frágil y el riesgo crece exactamente cuando ese eje mejore.

**10 issues candidatos propuestos por la campaña — PROPUESTOS, NO IMPLEMENTADOS, sin scoring CFPS individual** (siguiendo el mismo patrón que `SEARCH-MATCHING-QA-01` §4.3: la campaña de QA entrega severidad y evidencia; la priorización relativa y el scoring CFPS individual quedan para decisión posterior de Mario/ChatGPT):

| ID propuesto | Título | Sev. | Hallazgo relacionado |
|---|---|---|---|
| `CF-SEARCH-003` | Concentración en formas líquidas — segmento `conc:` en `presentationKey`, sin tocar `matchKey` | **P1** | QA-SEARCH-001 |
| `CF-WEB-002` | Ficha irresoluble con HTTP 200 — responder 404 real (quick win) + resolución persistida (decisión de arquitectura, depende de RFC-002/CFM-ID) | **P1** | QA-SEARCH-002 |
| `CF-DATA-001` | Laboratory / Brand Identity Quality — semántica del contrato, Salcobrand publica marca como laboratorio, mapeo muerto de Cruz Verde | P2 | QA-SEARCH-004 |
| `CF-DATA-002` | Tokens no-marca en identidad comercial — quick win en `isPlausibleCommercialIdentity()` | P2 | QA-SEARCH-005 |
| `CF-DATA-003` | Stock afirmado sin evidencia (Ahumada, EasyFarma) — cambio de contrato `hasStock`; si se implementa, incrementar `CACHE_PREFIX` (`CLAUDE.md` §11) | P2 | QA-SEARCH-006 |
| `CF-SEARCH-004` | Métrica de cobertura de comparación — instrumentar farmacias-por-tarjeta y grupos fragmentados | P2 | QA-SEARCH-003 |
| `CF-SEARCH-006` | Robustez de `unitCountKey` — 3 correcciones acotadas + tests, follow-up inmediato del PR ya mergeado | P2 | QA-SEARCH-008 |
| `CF-WEB-003` | Titular de mejor precio vs disponibilidad real — distinguir precio más bajo de precio más bajo comprable | P2/P3 | QA-SEARCH-006 |
| `CF-SEARCH-005` | EasyFarma: nombre truncado en origen — riesgo de multiplicar peticiones a un scraper frágil (R-009), decisión de arquitectura | P3→P2 | QA-SEARCH-007 |
| `CF-SEARCH-007` | Sinónimos de variante comercial — bajo impacto medido | P3 | QA-SEARCH-009 |

**Limitaciones honestas declaradas por la campaña** (ver `QA_SUMMARY.md` §9): datos observados post-merge (no se usó `?debug=1`); sin capturas de pantalla (navegación verificada por HTTP real, no visual); snapshot único de ~30 minutos (2026-08-31); Vercel Root Directory de `comparafarma-api` no verificable desde el repositorio.

**Relación con riesgos vigentes (`docs/program/RISKS.md`):** la campaña señala que R-009 (scrapers frágiles) queda agravado en evidencia, no en probabilidad — EasyFarma entrega dato degradado (QA-SEARCH-007) sin estar caído, algo que el monitor horario actual no detecta porque verifica disponibilidad, no calidad; y que R-007 (sin fuente confiable de bioequivalencia) tiene un efecto lateral cuantificado sobre la fragmentación de QA-SEARCH-003.
...
| 1.5 | 2026-08-31 | Activo | Pendiente (CEO/fundador) | Agregada iniciativa `CF-QA-001` (FASE 3, Product/Engineering) — campaña de QA "Search Product Identity & Price Comparison", con evidencia completa pusheada (no mergeada) en la branch `docs/qa-search-product-identity-001` (SHA `e09f9e2`). Registrados los 10 hallazgos (QA-SEARCH-001 a 010) con severidad y evidencia, la certificación `SAFE_TO_MERGE` de `fix/quantity-mismatch-false-merge` (ya mergeado a `origin/main`, SHA `48a7de5`) y los 10 issues candidatos propuestos por la campaña (`CF-SEARCH-003` a `007`, `CF-DATA-001` a `003`, `CF-WEB-002`/`003`), marcados explícitamente como propuestos, sin CFPS individual y sin implementación. Sin cambios de código ni ampliación de alcance en esta revisión documental. | `docs/qa/search-product-identity/QA_SUMMARY.md` y `cases/QA-SEARCH-001.md` a `010.md` (branch `docs/qa-search-product-identity-001`); `git log origin/main` para el merge `48a7de5` |
...
| 2026-08-31 | Registro de `CF-QA-001` (campaña de QA de identidad de producto y comparación de precios) con sus 10 hallazgos, la certificación `SAFE_TO_MERGE` de `fix/quantity-mismatch-false-merge` (ya mergeado, SHA `48a7de5`) y 10 issues candidatos propuestos, sin implementación | CTO / auditor de producto (análisis, no decisión) | `docs/program/MASTER_BACKLOG.md` v1.5 (este documento) |
