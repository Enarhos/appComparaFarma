...
**Nota de gobierno (2026-08-28 — decisión de Mario tras auditoría pre-PR de `CF-SEARCH-001`):** la branch `fix/cf-search-001-product-identity` (iniciativa `CF-SEARCH-001`, detalle debajo) ya ejecutó, sin declararlo explícitamente al momento de implementarse, **parte de V-2** — agrega forma farmacéutica como token nuevo en `presentationKey` (`|form:<f>`, `dosageFormClass`) — y la rotación de slugs correspondiente (**Gen 5**, con redirect 301), **antes del cierre formal del gate W-1** (la revisión de Google Play de Mobile vc33 no había cerrado a la fecha de esta nota) y sin la evaluación de riesgo SEO que el Paso 7 de esta secuencia exige formalmente antes de rotar slugs. Aclaración importante: esto **no es la migración completa de Option D** — `CF-SEARCH-001` no quita `bio:` de `presentationKey` (ese token sigue presente hoy, sin cambios), usa su propio mecanismo/nombre de token (`|form:`) diseñado independientemente para distinguir variantes comerciales, y no implementa `deriveGroupBioStatus` ni la política V-1 (ANY) ni el resto de los 8 pasos de esta secuencia. Es una ejecución **parcial y paralela** de un fragmento de V-2, no un adelanto de Option D completo.

Mario (Product Owner), tras revisar el hallazgo de auditoría (QA-06), decidió explícitamente **aceptar esta ejecución parcial sin revertirla y sin bloquear el PR de `CF-SEARCH-001`** por este motivo — no fue un descuido no revisado, fue una decisión consciente tomada con el hallazgo ya sobre la mesa. La consecuencia aceptada como costo conocido: cuando eventualmente se implemente el resto de Option D (quitar `bio:` de `presentationKey`, Paso 7 restante — el que sí requiere la evaluación de riesgo SEO todavía pendiente), habrá una **segunda rotación de slugs (Gen 6)**, adicional a la Gen 5 ya ejecutada por `CF-SEARCH-001`. El gate W-1 en sí (inicio del Paso 2 en adelante de esta iniciativa) sigue vigente y no se da por cerrado por este hallazgo — ver detalle de `CF-SEARCH-001` más abajo para el mecanismo concreto implementado y su cross-reference.
...
**Nota de gobierno — relación con `BIOEQUIVALENCE-DATA-QUALITY-01` (ver nota fechada 2026-08-28 en el detalle de esa iniciativa, arriba):** una auditoría pre-PR adversarial de esta branch encontró (hallazgo QA-06) que el token `|form:` (`dosageFormClass`) agregado a `presentationKey` y la rotación de slugs Gen 5 ejecutan, sin haberlo declarado explícitamente, parte de la decisión V-2 y la rotación de slugs del Paso 7 de `BIOEQUIVALENCE-DATA-QUALITY-01` — la misma iniciativa gateada por W-1 hasta el cierre de la revisión de Google Play vc33 — antes de ese cierre y sin la evaluación de riesgo SEO que el Paso 7 exige formalmente. Mario revisó el hallazgo y decidió aceptar la ejecución parcial sin revertir nada ni bloquear este PR por ese motivo; la consecuencia aceptada es una segunda rotación de slugs (Gen 6) futura cuando se implemente el resto de Option D (quitar `bio:` de `presentationKey`). La misma auditoría pre-PR encontró además 2 hallazgos P1 de calidad (QA-01: HTTP 500 en resolución de slugs ambiguos en Web; QA-02: `keyExtractor` de listas en Mobile), cuya corrección está en curso en una tarea separada (worktree `wt-cf-search-001`, sobre esta misma branch) — no confirmado como cerrado al momento de esta nota, por lo que el estado de esta iniciativa se mantiene sin cambios (implementado y testeado, pendiente de PR/review, no mergeado) hasta que esa corrección quede verificada.
...
| 1.5 | 2026-08-28 | Activo | Pendiente (CEO/fundador) | Registrada nota de gobierno (hallazgo QA-06 de auditoría pre-PR adversarial de `CF-SEARCH-001`): la branch `fix/cf-search-001-product-identity` ejecutó parcialmente V-2 (token `|form:` en `presentationKey`) y la rotación de slugs Gen 5 del Paso 7 de `BIOEQUIVALENCE-DATA-QUALITY-01`, antes del cierre formal del gate W-1 y sin la evaluación de riesgo SEO que ese paso exige. Mario revisó el hallazgo y decidió aceptar la ejecución parcial sin revertir nada ni bloquear el PR de `CF-SEARCH-001`, aceptando como costo conocido una segunda rotación de slugs (Gen 6) futura al implementar el resto de Option D. Agregada referencia cruzada equivalente en el detalle de `CF-SEARCH-001`, junto con mención de que 2 hallazgos P1 de esa misma auditoría (QA-01, QA-02) están en corrección en una tarea separada, sin confirmación de cierre. Sin reapertura de las decisiones V-1/V-2/W-1 ya cerradas; sin cambios de código. | Auditoría pre-PR adversarial de `fix/cf-search-001-product-identity` (hallazgo QA-06); decisión de Mario registrada en esta sesión |
...
| 2026-08-28 | Registro de nota de gobierno sobre ejecución parcial de V-2/Paso 7 de `BIOEQUIVALENCE-DATA-QUALITY-01` dentro de `CF-SEARCH-001` (hallazgo QA-06 de auditoría pre-PR), con decisión de Mario de aceptarla sin revertir y sin bloquear el PR, aceptando como costo conocido una segunda rotación de slugs (Gen 6) futura | CTO / auditor de producto (análisis, no decisión) — decisión de aceptación tomada por Mario | `docs/program/MASTER_BACKLOG.md` v1.5 (este documento) |
