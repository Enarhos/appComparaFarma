...
| **Documentos de los que depende** | `docs/design/system/DESIGN_SYSTEM.md`, `docs/design/system/GRID_SYSTEM.md`, `docs/design/system/SPACING_SYSTEM.md`, `docs/design/brand/BRAND_GUIDELINES.md`, `docs/design/brand/COLOR_SYSTEM.md`, `docs/design/brand/TYPOGRAPHY_SYSTEM.md`, `docs/design/brand/ICONOGRAPHY_SYSTEM.md`, `docs/design/brand/BRAND_FOUNDATIONS.md`, `docs/design/DESIGN_BRIEF.md`, `docs/governance/templates/GOVERNED_DOCUMENT_TEMPLATE.md` |
...
- Ninguna animación ni transición. Los principios de motion del isotipo ya están gobernados en `docs/design/brand/LOGO_SYSTEM.md` §4.8; este documento no los repite ni crea una filosofía de movimiento distinta.
...
- Ninguna responsabilidad funcional del color. Pertenece íntegramente a `docs/design/brand/COLOR_SYSTEM.md` (§4.6).
...
|---|---|---|
| Claridad | `docs/design/brand/BRAND_FOUNDATIONS.md` §11.1, Principio IV; `docs/design/DESIGN_BRIEF.md` §4.11 | Una jerarquía perceptual clara evita que la persona tenga que decidir por su cuenta qué mirar primero |
| Neutralidad | `docs/design/brand/BRAND_FOUNDATIONS.md` §11.2, §12; ya aplicada a color (`COLOR_SYSTEM.md` §4.5), espacio (`SPACING_SYSTEM.md` §4.2.3) y estructura (`GRID_SYSTEM.md` §4.8) | Cuarta y última aplicación del mismo principio a través de las Foundations de organización espacial y perceptual — desarrollado en §4.7 |
| Consistencia | `docs/design/brand/BRAND_ARCHITECTURE.md` §4.1 (Branded House); `docs/design/system/DESIGN_SYSTEM.md` §4.2, §4.8 | El mismo criterio de prioridad debe aplicarse igual en cualquier pantalla equivalente |
| Orientación | Concepto central de diseño (`docs/design/decisions/DESIGN_DECISION_LOG.md`, DD-001); ya aplicado espacialmente en `GRID_SYSTEM.md` §4.3 (Navegación) | Aplicado aquí no al espacio sino a la atención: saber qué mirar primero es tan parte de la orientación como saber dónde está cada cosa |
| Accesibilidad | `docs/design/brand/BRAND_FOUNDATIONS.md` §11.2; heredado de `TYPOGRAPHY_SYSTEM.md`, `COLOR_SYSTEM.md`, `ICONOGRAPHY_SYSTEM.md`, `SPACING_SYSTEM.md` y `GRID_SYSTEM.md` | Desarrollado en §4.8 |
| Simplicidad | `docs/design/brand/BRAND_FOUNDATIONS.md` §11.2; `docs/design/system/DESIGN_SYSTEM.md` §4.2 | El sistema debe resolver la prioridad con el menor número de niveles conceptuales necesarios (§4.2) |
...
El nivel más alto y menos frecuente del sistema: contenido que efectivamente interrumpe el flujo de la persona porque la alternativa —no interrumpir— sería peor para su bienestar o su decisión (por ejemplo, un error que impide completar una búsqueda). Debe ser el nivel menos usado de todo el sistema: su uso frecuente lo degrada, mismo principio ya aplicado al Color de Énfasis en `docs/design/brand/COLOR_SYSTEM.md` §4.2.5 (*"un sistema que recurre al énfasis con frecuencia deja de tener énfasis: lo diluye"*).
...
Esta es la aplicación más directa, dentro del Design System, de un principio ya consolidado en la identidad de marca: *"una buena experiencia ayuda a decidir. Nunca empuja a decidir"* (`docs/design/brand/BRAND_FOUNDATIONS.md` §15). Un sistema de elevación que amplifica algo por razones ajenas a su importancia funcional real ya está empujando la decisión, no ayudando a tomarla.
...
- **Ninguna farmacia puede recibir mayor elevación únicamente por acuerdos comerciales.** Consecuencia directa de `docs/design/brand/BRAND_FOUNDATIONS.md` §12 (*"no privilegiamos una farmacia por sobre otra por conveniencia comercial"*) y del Principio de producto 3, "Neutralidad" (§11.2).
...
| Prohibición de manipular la decisión de la persona | `docs/design/brand/BRAND_FOUNDATIONS.md` §15, §18 | ✔ — fundamenta §2 y §4.3 | — |
...
| Neutralidad entre farmacias | `docs/design/brand/BRAND_FOUNDATIONS.md` §12, §11.2; ya aplicada en `COLOR_SYSTEM.md` §4.5, `SPACING_SYSTEM.md` §4.2.3, `GRID_SYSTEM.md` §4.8 | ✔ — cuarta y última aplicación (§4.7) | — |
| Independencia antes que rentabilidad | `docs/design/brand/BRAND_FOUNDATIONS.md` §11.1, Principio VII; Constitución Art. IV | ✔ — fundamenta la regla sobre promociones comerciales (§4.7) | — |
| Restricción de uso del Color de Énfasis | `docs/design/brand/COLOR_SYSTEM.md` §4.2.5 | ✔ — extendida por analogía a Interrupciones excepcionales (§4.2.5 de este documento) | — |
| Relación Color–prioridad (refuerzo, no creación) | `docs/design/brand/COLOR_SYSTEM.md` §4.2.3, §4.2.5 | ✔ — formalizada en §4.6 | — |
...
| Principios de motion del isotipo | `docs/design/brand/LOGO_SYSTEM.md` §4.8 | Referenciados, no duplicados (§3) | La elevación no incluye principios de movimiento propios |
...
- `docs/design/brand/COLOR_SYSTEM.md` — sigue siendo la única fuente de las responsabilidades funcionales del color; este documento aclara que el color puede reforzar la prioridad que aquí se gobierna, nunca crearla (§4.6).
