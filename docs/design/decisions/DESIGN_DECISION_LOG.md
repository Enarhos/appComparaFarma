...
| DD-004 | 2026-08-16 | Aprobado | **Rebranding público del producto B2C: ComparaFarma → PreciosFarma** (dominio `preciosfarma.cl`, tagline "Compara precios de medicamentos"). La identidad visual ya aprobada (Concepto "Orientación", Candidato 09, paleta Indigo `#3F3FB8` / Brand Ink `#2E2E8C` / Accent Teal `#0D827B`, tipografía Inter, iconografía Lucide — DD-001/DD-002/DD-003) **se mantiene sin cambios**: este rebranding es exclusivamente de nombre verbal público, no de sistema visual. No se reabre ninguna decisión de forma, color o tipografía. | Decisión directa de Mario (CTO/Product Owner) + ChatGPT (CTO/Product), sesión 2026-08-16, posterior a una auditoría read-only de identidad visual que confirmó que el isotipo Candidato 09 no depende semánticamente del nombre "ComparaFarma" y que la paleta/tipografía aprobadas en `BRAND_EXPERIENCE_V1.md` nunca llegaron a implementarse en Mobile/Web, por lo que adoptar el nombre nuevo no tiene costo de migración visual adicional al que ya existía. Constituye el criterio de reapertura "Rebranding oficial" ya previsto en `docs/design/brand/DOMAIN_STATUS.md`, aplicado únicamente a la capa de nombre, no a la arquitectura visual. | Instrucción directa de Mario/ChatGPT, sesión 2026-08-16 ("PROYECTO: ComparaFarma → PreciosFarma") | Todo asset y texto público futuro debe usar "PreciosFarma" como nombre de marca. `ComparaFarma` se mantiene como identidad técnica interna (`mla.app.comparafarma`, `@comparafarma/domain`, repositorio, `comparafarma-api`, proyecto Vercel/Supabase existentes) por decisión explícita — ver `docs/design/brand/BRAND_ARCHITECTURE.md` §4.2 (actualización 2026-08-16). No se generó ninguna forma, color ni tipografía nueva. |
